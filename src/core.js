...
async function checkIfSuspended(resp, appstate) {
  try {
    const appstateCUser = appstate.find((i) => i.key === "c_user" || i.key === "i_user" || i.name === "c_user" || i.name === "i_user");
...
async function buildAPI(html, jar) {
  ...
  const primaryProfile = cookies.find((val) => val.name === "c_user" || val.key === "c_user" || (val.cookieString && typeof val.cookieString === "function" && val.cookieString().startsWith("c_user=")) || (typeof val === "string" && val.startsWith("c_user=")));
  const secondaryProfile = cookies.find((val) => val.name === "i_user" || val.key === "i_user" || (val.cookieString && typeof val.cookieString === "function" && val.cookieString().startsWith("i_user=")) || (typeof val === "string" && val.startsWith("i_user=")));

  if (!primaryProfile && !secondaryProfile) {
    const appState = JSON.parse(fs.readFileSync('account.txt', 'utf8'));
    const c_user = appState.find(i => i.key === 'c_user' || i.name === 'c_user');
    const i_user = appState.find(i => i.key === 'i_user' || i.name === 'i_user');
    if (c_user || i_user) {
        userID = i_user?.value || c_user?.value;
        const domain = ".facebook.com";
        const expires = new Date(Date.now() + 1000 * 60 * 60 * 24 * 365);
        appState.forEach(cookie => {
            const cookieStr = `${cookie.key || cookie.name}=${cookie.value}; expires=${expires.toUTCString()}; domain=${cookie.domain || domain}; path=${cookie.path || '/'};`;
            jar.setCookie(cookieStr, `https://www.facebook.com`);
        });
    }
  }

  if (!userID && !primaryProfile && !secondaryProfile) {
  ...
  userID = secondaryProfile?.value || primaryProfile?.value || secondaryProfile?.cookieString().split("=")[1] || primaryProfile?.cookieString().split("=")[1];
  if (!userID) {
    const rawCookies = jar.getCookies(fbLink());
    const cUser = rawCookies.find(c => c.key === "c_user");
    const iUser = rawCookies.find(c => c.key === "i_user");
    userID = iUser?.value || cUser?.value;
  }
  if (!userID) {
    throw new Error(ERROR_RETRIEVING);
  }
...
async function loginHelper(appState, apiCustomized, callback) {
  try {
    ...
    if (appState) {
      const cookies = Array.isArray(appState) ? appState : (typeof appState === 'string' ? JSON.parse(appState) : []);
      cookies.forEach(cookie => {
        const domain = cookie.domain || ".facebook.com";
        const expires = cookie.expirationDate ? new Date(cookie.expirationDate * 1000) : new Date(Date.now() + 1000 * 60 * 60 * 24 * 365);
        const cookieStr = `${cookie.key || cookie.name}=${cookie.value}; expires=${expires.toUTCString()}; domain=${domain}; path=${cookie.path || '/'};${cookie.secure ? ' secure;' : ''}${cookie.httpOnly ? ' httpOnly;' : ''}`;
    ...
    api.xenos = { ...(apiCustomized && { ...apiCustomized }) };
    ...
    utils.log("To check updates: you may check on https://github.com/LordXenos/XENOS-FCA");
