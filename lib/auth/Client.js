...
class XENOSClient {
  constructor(credentials, options = {}) {
    ...
    if (this.options.validateSession !== false) {
      try {
        ...
        this.logger.info('XENOSClient', 'Session validation passed', validation);
      } catch (err) {
        this.logger.error('XENOSClient', 'Session validation failed', { error: err.message });
        throw err;
      }
    }
  ...
  connect(callback) {
    this.logger.info('XENOSClient', 'Initiating connection...');
    
    legacyClient.login(this.credentials, this.options, (err, api) => {
      if (err) {
        this.logger.error('XENOSClient', 'Connection failed', { error: err.message });
        return callback(new AuthenticationError('Failed to connect', { originalError: err }));
      }
      
      this.logger.info('XENOSClient', 'Successfully connected');
          ...
          return originalListenMqtt.call(api, async (err, event) => {
            if (!err && event && this.webhook) {
              await this.webhook.sendEvent(event.type || 'unknown', event).catch(webhookErr => {
                this.logger.warn('XENOSClient', 'Webhook delivery failed', { error: webhookErr.message });
              });
            }
            ...
            if (this.plugins && event) {
              try {
                ...
                this.logger.error('XENOSClient', 'Middleware error', { error: middlewareErr.message });
              }
            }
